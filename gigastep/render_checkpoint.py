...
from mava.wrappers import AgentIDWrapper, AutoResetWrapper, GigastepWrapper
...
def render(config: DictConfig):
    ...
    env = AutoResetWrapper(
        AgentIDWrapper(GigastepWrapper(scenario.make(**config.env.kwargs), has_global_state=True))
    )
    ...
    loaded_checkpoint = Checkpointer(
        model_name=config.logger.system_name, rel_dir="checkpoints", checkpoint_uid="20250821092339"
    )
