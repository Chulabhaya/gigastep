...
class GigastepFitness(object):
    ...
    def rollout_pmap(
        ...
        ego_policy_params: chex.ArrayTree,
        ado_policy_params: chex.ArrayTree,
    ):
        ...
        steps_re = out_rollout_map[1].reshape(
            -1, self.num_rollouts
        )  # NOTE: weird shape, but will only be summed
        if self.test:
    ...
    def rollout_ffw(
        ...
        def policy_step(state_input, tmp):
            ...
            obs, state, ego_policy_params, ado_policy_params, rng, cum_reward, valid_mask = (
                state_input
            )
            ...
            next_o, next_s, reward, dones, done = self.env.step(state, action, rng_step)
            ...
            new_cum_reward = cum_reward + (reward * valid_mask)[..., : self.ego_team_size].mean(
                axis=-1
            )
    ...
    def rollout_rnn(
        ...
        def policy_step(state_input, tmp):
            ...
            hidden_ego, action_ego, action_info = self.network(
                ego_policy_params, obs_ego, hidden_ego, rng_net
            )
            hidden_ado, action_ado, _ = self.network(
                ado_policy_params, obs_ado, hidden_ado, rng_net
            )
            action = jnp.concatenate((action_ego, action_ado), axis=0)
