...
def get_action_jax_opponent(
    actor_critic, actor_critic_opponent, obs, n_ego_agent, recurrent_hidden_states, device
):
    obs = torch.tensor(np.asarray(obs), device=device)
    obs = torch.unsqueeze(obs, 0)
    obs = torch.moveaxis(obs, -1, 2)
...
def evaluation_jax(
    ...
    env = make_scenario(env_name, **vars(env_cfg))
    rng = jax.random.PRNGKey(3)
    key_reset, rng = jax.random.split(rng, 2)

    recurrent_hidden_states = torch.zeros(
        env.n_teams[0], actor_critic.recurrent_hidden_state_size, device=device
    )
    recurrent_hidden_states_opponent = torch.zeros(
        env.n_teams[1], actor_critic.recurrent_hidden_state_size, device=device
    )
    ...
    for i in range(num_of_evaluation):
        ...
        while True:
            ...
            if not random_action:
                action, recurrent_hidden_states = get_action_jax(
                    actor_critic, obs, env.n_teams[0], recurrent_hidden_states, device
                )

                action_opponent, recurrent_hidden_states_opponent = get_action_jax_opponent(
                    actor_critic,
                    actor_critic_opponent,
                    obs,
                    env.n_teams[0],
                ...
                if discrete_actions:
                    ...
                    action_opponent_jax = jnp.array(
                        action_opponent.int().detach().cpu().numpy().astype(np.int32)
                    )
                else:
                    action_jax = jnp.array(action.detach().cpu().numpy().astype(np.float32))
                    action_opponent_jax = jnp.array(
                        action_opponent.detach().cpu().numpy().astype(np.float32)
                    )

                action = jnp.concatenate([action_jax, action_opponent_jax], axis=0)
            else:
                action = jax.random.uniform(
                    key_action, shape=(env.n_agents, 3), minval=-1, maxval=1, dtype=jnp.float32
                )
    ...
    print(
        f"team1 win {wining_team}, team2 win {num_of_evaluation - wining_team} in total {num_of_evaluation}"
    )
...
if __name__ == "__main__":
    ...
    actor_critic, _ = torch.load(
        "trained_models/2v2/ppogigastepgiga20230523-105128.027506.pt",
