...
def main(**sweep_dict):
    ...
    from gigastep.evaluator import loop_env_vectorized
    ...
    if args.log_to_wandb:
        ...
        wandb.login(key=yaml.safe_load(open("learning/torch_ppo/.secret.yml", "r"))["WANDB_KEY"])

        config_del = vars(args)
        config_del.update(vars(env_cfg))

        wandb.init(
            name=exp_prefix, group=args.variant_name, project=project_name, config=config_del
        )
    ...
    if "gigastep" in args.env_name:
        ...
        envs = make_scenario(args.env_sub_name, **vars(env_cfg))
        ...
        if envs._obs_type == "rgb":
            envs.observation_space = Box(
                ...
                high=255 * jnp.ones([3, envs.resolution[0], envs.resolution[1]], dtype=jnp.uint8),
            )
        n_ego_agents = 0
        n_opponents = 0
    ...
    if os.path.exists(PATH):
        actor_critic, obs_rms = torch.load(PATH, map_location="cpu")
    else:
        if "gigastep" in args.env_name:
    ...
    if args.gail:
        ...
        file_name = os.path.join(
            args.gail_experts_dir, "trajs_{}.pt".format(args.env_name.split("-")[0].lower())
        )
    ...
    if "gigastep" in args.env_name:
        ...
        rollouts_opponent = RolloutStorage(
            args.num_steps,
            args.num_processes,
            envs.observation_space.shape,
            envs.action_space,
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            with torch.no_grad():
                ...
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
                    ...
                    masks_agent=rollouts.masks_agent[step].to(device),
                )
                (
                    value_opponent,
                    action_opponent,
                    action_log_prob_opponent,
                    recurrent_hidden_states_opponent,
                ) = actor_critic_opponent.act(
                    rollouts_opponent.obs[step].to(device),
                    rollouts_opponent.recurrent_hidden_states[step].to(device),
                    rollouts_opponent.masks[step].to(device),
                    masks_agent=rollouts_opponent.masks_agent[step].to(device),
                )
            ...
            if "gigastep" in args.env_name:
                ...
                obs, state, reward, agent_done, episode_done = envs.v_step(
                    state, action_jax, key_step
                )
                ...
                reward_episode_env_opponent += torch.sum(
                    reward_raw[:, n_ego_agents:], dim=-1, keepdim=True
                )
                ...
                for i in range(done.shape[0]):
                    if done[i]:
                        infos[i] = {
                            "episode": {
                                "r": reward_episode_env[i, 0].detach().cpu().numpy(),
                                "r_opponent": reward_episode_env_opponent[i, 0]
                                .detach()
                                .cpu()
                                .numpy(),
        ...
        with torch.no_grad():
            next_value = actor_critic.get_value(
                rollouts.obs[-1].to(device),
                rollouts.recurrent_hidden_states[-1].to(device),
                rollouts.masks[-1].to(device),
                rollouts.masks_agent[-1].to(device),
            ).detach()
            next_value_opponent = actor_critic_opponent.get_value(
                rollouts_opponent.obs[-1].to(device),
                rollouts_opponent.recurrent_hidden_states[-1].to(device),
                rollouts_opponent.masks[-1].to(device),
                rollouts_opponent.masks_agent[-1].to(device),
            ).detach()
        ...
        rollouts_opponent.compute_returns(
            next_value_opponent,
            args.use_gae,
            args.gamma,
            args.gae_lambda,
        ...
        value_loss_opponent, action_loss_opponent, dist_entropy_oppoent = agent_opponent.update(
            rollouts_opponent
        )
        ...
        if j % args.log_interval == 0 and len(episode_rewards) > 1:
            ...
            if args.log_to_wandb:
                wand_log = {
                    ...
                    "last training episode": len(episode_rewards),
                    "reward": np.mean(episode_rewards),
                    "dist_entropy": dist_entropy,
                    "value_loss": value_loss,
                    "action_loss": action_loss,
                    "value_loss_opponent": value_loss_opponent,
                    "action_loss_opponent": action_loss_opponent,
        ...
        if (j % args.eval_interval == 0) and (
            "giga" in args.env_name
            ...
            if "giga" in args.env_name:
                for i in range(2):
                    ...
                    videos, wining_rate = evaluation_jax(
                        args.env_sub_name,
                        env_cfg.obs_type,
                    ...
                    if rate_elo:
                        ...
                        rate_cls = Rating(
                            device, args.path_pool, args.env_sub_name, env_cfg, num_match=5
                        )
                        for i in range(1):
                            rate_0 = rate_cls.rate_policy(
                                actor_critic, rate_0, add_policy_to_pool=False
                            )
                        if args.log_to_wandb:
                            wandb.log(
                                {
                ...
                env_evaluation = make_scenario(args.env_sub_name, **vars(env_cfg))
                wining_rate_vec_evaluation = loop_env_vectorized(
                    env=env_evaluation, policy=actor_critic, device=device
                )
                if args.log_to_wandb:
                    wandb.log(
                        {
...
def tune_args(args, sweep_dict=None):
    ...
    args.eval_interval = args.num_env_steps // args.num_steps // args.num_processes // 100
