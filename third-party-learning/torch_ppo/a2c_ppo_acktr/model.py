        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            if "Giga_distri" in model_kwargs.keys():
                if model_kwargs["giga_distri"]:
                    self.dist = eval(model_kwargs["distribution"])(
                        self.base.output_size, num_outputs, model_kwargs["num_agent"]
                    )
...
class CNNBase(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=512):
        ...
        self.main = nn.Sequential(
            init_(nn.Conv2d(num_inputs, 32, kernel_size=2, stride=4)),
            nn.ReLU(),
            init_(nn.Conv2d(32, 64, kernel_size=2, stride=2)),
            nn.ReLU(),
            init_(nn.Conv2d(64, 32, kernel_size=2, stride=1)),
            nn.ReLU(),
            Flatten(),
        )

        self.main_down = nn.Sequential(init_(nn.Linear(96, hidden_size)), nn.ReLU())
...
class Giga_distri_cnn(MLPBase):
    def __init__(
        ...
        observation_type="vec position",
        use_cnn=True,
    ):
        num_inputs_agent = (
            int(num_inputs[0]) if len(num_inputs) == 1 else int(num_inputs[0] * num_inputs[1])
        )
        super(Giga_distri_cnn, self).__init__(
            num_inputs=num_inputs_agent,
            recurrent=recurrent,
        ...
        if self.use_cnn:
            self.backbone = nn.Sequential(
                init_(nn.Conv2d(3, 128, kernel_size=8, stride=4)),
                nn.ELU(),
                init_(nn.Conv2d(128, 64, kernel_size=8, stride=6)),
                nn.ELU(),
                Flatten(),
                init_(nn.Linear(576, hidden_size)),
                nn.ELU(),
            )
            self.actor = nn.Sequential(init_(nn.Linear(hidden_size, hidden_size)), nn.ELU())

            self.critic_output_is_1 = nn.Sequential(init_(nn.Linear(hidden_size, 1)), nn.ELU())
    ...
    def init_hidden_variable(self, inputs):
        rnn_hxs = torch.zeros(
            (inputs.shape[0] * inputs.shape[1], self.hidden_size), device=self.device
        )
        return rnn_hxs
    ...
    def forward(self, x, rnn_hxs, masks, masks_agent=None):
        ...
        if self.is_recurrent:
            ...
            x, rnn_hxs = self._forward_gru(
                x,
                rnn_hxs.view(x_shape[0] * x_shape[1], -1),
...
class Giga_distri_mlp(MLPBase):
    def __init__(
        ...
        super(Giga_distri_mlp, self).__init__(
            num_inputs=num_inputs_agent,
            recurrent=recurrent,
            recurrent_input_size=hidden_size,
            hidden_size=hidden_size,
        ...
        self.backbone = nn.Sequential(
            init_(nn.Linear(self.num_inputs[0], hidden_size)),
            nn.ELU(),
            init_(nn.Linear(hidden_size, hidden_size)),
            nn.ELU(),
            init_(nn.Linear(hidden_size, hidden_size)),
